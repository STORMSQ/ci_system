 ...
 $value=explode(';',$_POST['detail']);
   ...
   foreach($value as $row){
    ...
    $article_Num=count($total);
        ...
        <?php         
            $result_1 = $pdo->query("SELECT ".ARTICLEID.",".CHITITLE.", ".ENGTITLE.", volumeAndIssueId FROM ".ARTICLE." WHERE ".ARTICLEID." = '".$data."'");
                 $row_1=$result_1->fetch(PDO::FETCH_NUM);
                 $vol=substr($row_1[3],0,strlen($row_1[3])-1);
                 $issue=substr($row_1[3],-1,1);
             echo '<ul>';    
             echo '<li><b><font size="3">'.$vol.'卷'.$issue.'期--'.$row_1[1].$row_1[2].'</font></b></li>';
             echo '</ul>';    
