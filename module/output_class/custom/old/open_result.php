...
$result =$pdo->query("SELECT concat(volume,issue)as volandissue FROM `article_date` WHERE date_format(publicationDate,'%Y') ='".$_POST['year1']."' group by volandissue order by volandissue asc");
$row=$result->fetch(PDO::FETCH_NUM);

$range_1=$row[0].'00';
$result =$pdo->query("SELECT concat(volume,issue)as volandissue FROM `article_date` WHERE date_format(publicationDate,'%Y') ='".$_POST['year2']."' group by volandissue order by volandissue asc");
$row=$result->fetch(PDO::FETCH_NUM);
...
foreach($_POST['condition_select'] as $key=> $row){
   ...
   if($_POST['condition_select'][$key]!='null'){
    ...
    }elseif($_POST['condition_select'][$key]=='all'){ 
...
$sql='SELECT '.$action_2_left.$action_2_left_sep.$action_1_count.' FROM '.$table.' WHERE ('.$action_1.') '.$action_2_right.' '.$action_2_right_groupby;
#######################################################################################################
echo $sql;
...
echo '<tr>';
        ...
        for($i=0; $i<$result->columnCount(); $i++){
                ...
                echo '<th data-resizable-column-id="'.$field["name"].'">'.preg_replace($eng,$chi,$field["name"])."</th>";
...
if(count($row[$a]) %2 > 0 ){
     ...
     echo '<td>'.preg_replace($eng,$chi,$row[$a][$keys[0]]).'</td>';
...
function pie_chart($data,$column, $tag,$title){
...
if($_POST['groupby']==''){
?>
<div id="piechart1" style="width:100%; height: 960px;  float:left;"><?php echo pie_chart($data,$column, 'piechart1','結果數量一覽'); ?></div>
 
<?php

}else{//elseif($type=='multi_result'){
?>
