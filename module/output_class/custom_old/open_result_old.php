...
foreach($_POST['condition_select'] as $key=> $row){
   ...
   if($_POST['condition_select'][$key]!='null'){
      if($key==0){
      $action_2_left=$row;
      $action_2_right_groupby=' GROUP BY '.$row.' ';
...
foreach($action_1_tmp as $row){
 ...
 $action_1_count.='count(if('.$row.', true, null)) AS '."'$action_1_name[$l]'".' , group_concat(if('.$row.',tb.REFID,null) separator ";") AS "group-'.$action_1_name[$l].'"';
...
$sql='SELECT '.$action_2_left.$action_2_left_sep.$action_1_count.' FROM '.$action_1.' WHERE '.$action_2_right.' '.$action_2_right_groupby;
#######################################################################################################
echo $sql;
...
function pie_chart($data,$column, $tag,$title,$total){
...
?>
<div id="piechart1" style="width:90%; height: 800px;  float:left;"><?php echo pie_chart($data,$column, 'piechart1','結果數量一覽（搜尋結果量，共'.$rowCount.'筆為總數）',''); ?></div>
<!--<div id="piechart2" style="width:50%; height: 1024px;  float:left;"><?php //echo pie_chart($data,$column, 'piechart2','結果數量一覽（全部資料量，共'.$col_num.'筆為總數）',$total); ?></div>  -->
<?php

}//elseif($type=='multi_result'){
?>
